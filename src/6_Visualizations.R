...
#load(file =  here("Object" "LDA_predict","LDA_model_bow25.Rdata"))
...
bow_test_predictions['type'] <- rep("test",2769)
...
bow_test_predictions2['type'] <- rep("test",2769)
...
#load(file = "D:\\DKE-sem\\sem2\\DWR\\project\\github-rep-test\\data-science-with-r\\Object_files\\LDA-sim\\main.Rdata")
rep_df <- main_df
...
colnames(bow_test_train_model1) <- c('X1','X2',paste("Topic",c(1:25),sep=" "),'topic_highest_prob','type','url','title','keywords')
tsne <- Rtsne(bow_test_train_model1[,1:25], perplexity = 30, pca = FALSE, check_duplicates = FALSE)
df <- tsne$Y
colnames(df) <- c("X1","X2")
bow_test_train_model1 <- cbind(df,bow_test_train_model1)
load(file = "D:\\DKE-sem\\sem2\\DWR\\project\\github-rep-test\\data-science-with-r\\Object_files\\LDA-sim\\doc_sim_df.Rdata")
